...
FILE2="/groups/umcg-lifelines/tmp02/projects/ov23_0782/jtuinman/references/ldsc/Plaque.ldsc.txt"
...
################################################################################
#Make sure the files have at least (bare minimum) the following columns: SNP, A1, A2, P
...
python $LDSC_DIR/ldsc.py \
    --rg logCAC.clean.sumstats.gz,plaque.clean.sumstats.gz \
    --ref-ld-chr eur_w_ld_chr/ \
    --w-ld-chr eur_w_ld_chr/ \
    --out log_vs_plaque_rg
...
cp logCAC_h2* $OUTDIR/
cp plaque_h2* $OUTDIR/
cp log_vs_plaque_rg* $OUTDIR/
cp logCAC.clean.sumstats.gz $OUTDIR/
cp plaque.clean.sumstats.gz $OUTDIR/
cp $FILE1 $OUTDIR/
cp $FILE2 $OUTDIR/
